...
import { ClipboardList, Calendar, Award, FileText, Send } from 'lucide-react';
...
const TeacherPortal = () => {
  ...
  const sidebarTabs = [
    ...
    { id: 'marks', label: 'Enter Grades', icon: Award },
    { id: 'assignments', label: 'Assignments Desk', icon: FileText },
    { id: 'notice', label: 'Publish Notice', icon: Send }
  ];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 min-h-[calc(100vh-80px)] bg-slate-50 dark:bg-slate-950 font-sans">
      <aside className="lg:col-span-1 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-805 p-6 flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-3 pb-6 border-b border-slate-200 dark:border-slate-800 mb-6">
            ...
            <div className="overflow-hidden">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold block mt-0.5">Dept HOD: {currentUser?.department}</span>
            </div>
          </div>
        ...
        <div className="hidden lg:block p-4 bg-slate-50 dark:bg-slate-950/40 rounded-xl text-xs text-slate-450 border border-slate-200/50 dark:border-slate-850 leading-relaxed mt-6">
          Instructor Roster ID: <strong className="text-slate-700 dark:text-slate-350">{currentUser?.id}</strong><br />
          Phone: <strong className="text-slate-700 dark:text-slate-350">{currentUser?.phone}</strong>
        </div>
      </aside>
      <main className="lg:col-span-3 p-6 md:p-10 overflow-y-auto">
        <div className="flex justify-between items-center mb-8 gap-4 flex-wrap border-b border-slate-200/50 dark:border-slate-850 pb-4">
          <div>
            <span className="text-xs font-bold uppercase tracking-widest text-accent">Teacher Dashboard</span>
          ...
          <span className="text-xs font-bold bg-accent-light text-accent py-1 px-3.5 rounded-full border border-accent/15">Instructor Account: Active</span>
        </div>
        <Outlet />
      </main>
    </div>
